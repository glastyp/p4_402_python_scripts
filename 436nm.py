...
plt.title("Kennlinie für $436\,\mathrm{nm}$; 1. Messung",
          y=1.08)
...
fig_U.savefig("436nm1.pdf")

""" 2 """

data = open("436nm2.txt", 'r')
lines=data.readlines()
I=[]
dI=[]
U_G=[]
...
plt.title("Kennlinie für $436\,\mathrm{nm}$; 2. Messung nach neuer Justage",
          y=1.08)
plt.xlabel("$U_\mathrm{G}/\,\mathrm{mV}$")
plt.ylabel("$\sqrt{I-I_0}/\,\sqrt{\mathrm{V}}$")

fit_params, pcov = scipy.optimize.curve_fit(f, U_G[:-4], I[:-4], sigma=dI[:-4])


print(fit_params)
perr = np.sqrt(np.diag(pcov))
print(perr)
print(chi2(U_G[:-4], I[:-4], dI[:-4], f, fit_params[0], fit_params[1]))
