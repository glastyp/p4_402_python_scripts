...
plt.title("Kennlinie für $365\,\mathrm{nm}$; 1. Messung",
          y=1.08)
plt.xlabel("$U_\mathrm{G}/\,\mathrm{mV}$")
plt.ylabel("$\sqrt{I-I_0}/\,\sqrt{\mathrm{V}}$")

fit_params, pcov = scipy.optimize.curve_fit(f, U_G[:-5], I[:-5], sigma=dI[:-5])


print(fit_params)
perr = np.sqrt(np.diag(pcov))
print(perr)
print(chi2(U_G[:-5], I[:-5], dI[:-5], f, fit_params[0], fit_params[1]))
...
fig_U.savefig("365nm1.pdf")

""" 2 """

data = open("365nm2.txt", 'r')
lines=data.readlines()
I=[]
dI=[]
U_G=[]
...
plt.title("Kennlinie für $365\,\mathrm{nm}$; 2. Messung nach neuer Justage",
          y=1.08)
plt.xlabel("$U_\mathrm{G}/\,\mathrm{mV}$")
plt.ylabel("$\sqrt{I-I_0}/\,\sqrt{\mathrm{V}}$")
...
fitx = np.linspace(U_G[0],U_G[-1],num=10000)
fity = f(fitx, *fit_params)
...
fig_U.savefig("365nm2.pdf")

""" 3 """

data = open("365nm3.txt", 'r')
lines=data.readlines()
I=[]
dI=[]
U_G=[]
...
plt.title("Kennlinie für $365\,\mathrm{nm}$; 3. Messung, höhere Intensität",
          y=1.08)
plt.xlabel("$U_\mathrm{G}/\,\mathrm{mV}$")
plt.ylabel("$\sqrt{I-I_0}/\,\sqrt{\mathrm{V}}$")

fit_params, pcov = scipy.optimize.curve_fit(f, U_G[:-7], I[:-7], sigma=dI[:-7])


print(fit_params)
perr = np.sqrt(np.diag(pcov))
print(perr)
print(chi2(U_G[:-7], I[:-7], dI[:-7], f, fit_params[0], fit_params[1]))
